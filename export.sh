...
while read id name; do
  target="$name-wordpress-${timestamp}.xml"
  ...
  wget --secure-protocol=auto \
     ...
     --max-redirect=0\
     "${wpblog}/wp-admin/export.php?download=true&content=posts&cat=0&post_author=$id&post_start_date=0&post_end_date=0&post_status=publish&page_author=0&page_start_date=0&page_end_date=0&page_status=0&submit=Download+Export+File"
done < $users
